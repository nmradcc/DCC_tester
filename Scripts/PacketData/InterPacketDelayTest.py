...
def run_inter_packet_delay_test(rpc, loco_address, inter_packet_delay_ms=5):
    """
    Run the inter-packet delay test.
    
    Args:
        rpc: DCCTesterRPC client instance
        loco_address: Locomotive address
        inter_packet_delay_ms: Delay between packets in milliseconds (default: 5ms per NEM 671)
    ...
    print("=" * 70)
    print("DCC Inter-Packet Delay Test (NEM 671)")
    ...
    try:
        ...
        # Step 5: Wait for inter-packet delay (NEM 671 specifies minimum 5ms)
        ...
        # Step 6: Wait for motor to run and stabilize
        print("Step 6: Waiting for motor to run and stabilize (500ms)...")
        time.sleep(0.5)
        ...
        _on_current_ma = response.get("current_ma", 0)
        print(f"✓ Motor run current: {motor_on_current_ma} mA\n")

        # Step 8: Wait for inter-packet delay before emergency stop
        print(f"Step 8: Waiting {inter_packet_delay_ms} ms (inter-packet delay before stop)...")
        time.sleep(inter_packet_delay_ms / 1000.0)
        print(f"✓ Inter-packet delay complete\n")
        
        # Step 9: Send emergency stop packet
        print(f"Step 9: Sending emergency stop packet to address {loco_address}...")
        ...
        print("Step 10: Reading motor stopped current...")
        response = rpc.send_rpc("get_current_feedback_ma", {})
        ...
        motor(f"✓ Motor stopped current: {motor_stopped_current_ma} mA\n")
        ...
        # Pass criteria: motor current must increase during run and decrease after stop
        test_pass = (current_increase > 0 and current_decrease > 0)
        ...
        print(f"  6. Waited 500 ms for motor to stabilize")
        print(f"  7. Read motor run current: {motor_on_current_ma} mA")
        print(f"  8. Waited {inter_packet_delay_ms} ms (inter-packet delay)")
        print(f"  9. Sent emergency stop packet to address {loco_address}")
        ...
        print(f"\nPass Criteria:")
        print(f"  Current increased during run: {current_increase > 0} ({current_increase:+d} mA)")
        print(f"  Current decreased after stop: {current_decrease > 0} ({current_decrease:+d} mA)")
        print()
...
def main():
    ...
    try:
        ...
        # Run the test
        result = run_inter_packet_delay_test(
            rpc, 
            LOCO_ADDRESS, 
